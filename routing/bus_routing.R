url <- paste("https://api.tfl.gov.uk/Journey/JourneyResults/", start_lat, ",", start_lon, "/to/", end_lat, ",", end_lon,
             "?journeyPreference=LeastTime&mode=bus&app_id=", tfl_app_id, "&app_key=", tfl_app_key ,sep="")
...
if (json_data$`$type` == 'Tfl.Api.Presentation.Entities.JourneyPlanner.ItineraryResult, Tfl.Api.Presentation.Entities') {
 ...
 # temp_results_frame$line <- fill_missing_info(temp_results_frame$line)
  ...
  print(paste("At", Sys.time(), "routing unique journey id", i, "was completed using TfL bus mode"))
  unique_journeys[i,]$status <- 'tfl'
} else {
  print(paste("At", Sys.time(), "routing unique journey id", i, "was NOT completed using TfL bus mode, the error id will be logged"))
  error_ids[j,] <- i
  j <- j+1
}  } else
{ print(paste("At", Sys.time(), "routing unique journey id", i, "was NOT completed using TfL bus mode, the error id will be logged"))
